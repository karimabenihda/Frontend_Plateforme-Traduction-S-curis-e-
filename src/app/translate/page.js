...
function page() {
  const [options, setOptions] = useState("français");
  const [text, setText] = useState("");
  const [translated_text, setTranslated_text] = useState("");
  // const [isFrFirst,setIsFrFirst]=useState(true)

  const leftLabel = options === "français" ? "English" : "Français";
  const leftValue = options ===  "français" ? "english" : "français";

  const rightLabel = options === "français" ? "Français" : "English";
  const rightValue = options ==="français" ? "français" : "english";
 ...
 const handleTranslate = async () => {
  ...
  try {
    let endpoint =
      options === "français"
        ? "http://127.0.0.1:8000/en_to_fr"
        : "http://127.0.0.1:8000/fr_to_en";
    ...
    setTranslated_text(response.data.translated_text);
  } catch (error) {
    console.log(error);
  }
};
...
const switchLanguages = () => {
    setOptions((prev) => (prev === "français" ? "english" : "français"));
  };
  ...
  <div className="space-y-4 w-full flex flex-col items-center">
    {/* Language Switch */}
    <div className="w-[50%] flex items-center justify-between bg-white p-1 border border-gray-950/50 rounded-full text-sm">
      ...
      <div className="flex items-center">
        <input
          ...
          checked={options === leftValue}
          onChange={(e) => setOptions(e.target.value)}
        />
        <label
          htmlFor={leftValue}
          className="cursor-pointer rounded-full py-2 px-8 text-gray-600 transition-colors duration-200 peer-checked:bg-[#4f39f6] peer-checked:text-white"
        >
          {leftLabel}
        </label>
      </div>
      ...
      <div className="flex items-center">
        <input
          ...
          id={rightValue}
          className="hidden peer"
          checked={options === rightValue}
          onChange={(e) => setOptions(e.target.value)}
        />
        <label
          htmlFor={rightValue}
          className="cursor-pointer rounded-full py-2 px-8 text-gray-600 transition-colors duration-200 
          peer-checked:bg-[#4f39f6] 
...
<div className=''>
<div className="relative w-[32rem] flex flex-col lg:flex-row gap-3">
...
<textarea 
 ...
 ></textarea>
    ...
    <div className="flex w-full justify-end gap-2 py-1.5">
      <button onClick={Clear}
        className="select-none rounded-md bg-white !text-[#4f39f6] py-2 px-4 text-center align-middle font-sans text-xs font-bold  text-white shadow-md shadow-gray-900/10 transition-all hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
        type="button">
        Clear
      </button>
      <button onClick={handleTranslate}
        className="select-none rounded-md bg-[#4f39f6] py-2 px-4 text-center align-middle font-sans text-xs font-bold  text-white shadow-md shadow-gray-900/10 transition-all hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
        type="button">
        Translate
      </button>
 ...
 <div className="relative w-full min-w-[200px]">
    ...
    <label
      ...
      <span className="absolute transform -translate-x-1/2 -translate-y-1/2 -top-20 -right-120">
